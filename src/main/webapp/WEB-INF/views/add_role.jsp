				...
				<div class="input-group">
					<label class="input-label">权限：</label> <label class="input-content">
						<c:forEach items="${jurisdictions }" var="item">
							<input type="checkbox" name="jurisdictionNames" class="checkbox" value="${item.name }">${item.name }　
						</c:forEach>
					</label>
				</div>
			...
			<form action="saveRole" method="post">
				...
				<div class="input-group">
					<label class="input-label">权限：</label> <label class="input-content">
						<c:forEach items="${jurisdictions }" var="item">
							<c:choose>
								<c:when test="${role.jurisdictions eq jurisdictions}">
								<input type="checkbox" name="jurisdictionNames" class="checkbox" value="${item.name }" checked>${item.name }　
								</c:when>
								<c:otherwise>
								<input type="checkbox" name="jurisdictionNames" class="checkbox" value="${item.name }">${item.name }　
								</c:otherwise>
